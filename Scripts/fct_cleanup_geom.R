...
cleanup_geom <- function(dataframe){
  ...
  fixed.dataframe <- tidyr::extract(
    dataframe,
    geom,
    c("longitude", "latitude"), # inverse ici : la première valeur est la longitude, la deuxième est la latitude
    "\\(\\((.*)\\s(.*)\\)\\)"
  ) # sépare la longitude et la latitude en deux colonnes distinctes
  
  fixed.dataframe[, c("latitude", "longitude")] <- sapply(
    fixed.dataframe[, c("latitude", "longitude")],
    as.numeric
  ) # converti en valeurs numériques
