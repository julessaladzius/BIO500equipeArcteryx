...
```{r}
#### Requête des données de taxonomie et d'abondance pour les populations de Aves, Mammalia et Amphibia, avec un calcul de la variation interannuelle moyenne ####
...
FROM 
    growth g
WHERE g.class IN ('Aves','Mammalia','Amphibia')
ORDER BY 
    g.class, g.cle_pop, g.years
;"

yoy_growth <- dbGetQuery(connexion,data_graph)
```
...
ggplot(yoy_growth, aes(x=years, y=moy_growth_yoy, group=class,color=class)) +
  ylab("Variation interannuelle moyenne (%)") +
  xlab("Années") +
  labs(title="Fluctuations des populations des principaux taxons terrestres au Québec",color="Classe") +
